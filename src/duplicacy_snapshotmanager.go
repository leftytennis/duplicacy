...
func (manager *SnapshotManager) PruneSnapshots(selfID string, snapshotID string, revisionsToBeDeleted []int,
	...
	dryRun bool, deleteOnly bool, collectOnly bool, threads int, keepDays int) bool {

	LOG_DEBUG("DELETE_PARAMETERS",
		"id: %s, revisions: %v, tags: %v, keepDays: %d, retentions: %v, exhaustive: %t, exclusive: %t, "+
			"dryrun: %t, deleteOnly: %t, collectOnly: %t",
		snapshotID, revisionsToBeDeleted, tags, keepDays, retentions,
	...
	for id, snapshots := range allSnapshots {
		...
		if len(revisionsToBeDeleted) > 0 {
			...
			// keepDays is specified with one or more tags
		} else if keepDays != -1 && len(tags) > 0 {
			for i := 0; i < len(tags); i++ {
				...
				for k := 0; k < len(tag_snapshots); k++ {
					days := getDaysBetween(tag_snapshots[k].StartTime, time.Now().Unix())
					if days > keepDays {
						LOG_INFO("SNAPSHOT_KEEP_PRUNE", "Pruning snapshot ID: %s, revision: %d, tag: %s, days: %d", tag_snapshots[k].ID, tag_snapshots[k].Revision, tag_snapshots[k].Tag, days)
						tag_snapshots[k].Flag = true
						toBeDeleted++
					} else {
						LOG_DEBUG("SNAPSHOT_KEEP_PRUNE", "Keeping snapshot ID: %s, revision: %d, tag: %s, days: %d", tag_snapshots[k].ID, tag_snapshots[k].Revision, tag_snapshots[k].Tag, days)
					}
				}
			}
			// keepDays is specified without any tags
		} else if keepDays != -1 {
			for i := 0; i < len(snapshots); i++ {
				days := getDaysBetween(snapshots[i].StartTime, time.Now().Unix())
				if days > keepDays {
					LOG_INFO("SNAPSHOT_KEEP_PRUNE", "Pruning snapshot ID: %s, revision: %d, tag: %s, days: %d", snapshots[i].ID, snapshots[i].Revision, snapshots[i].Tag, days)
					snapshots[i].Flag = true
					toBeDeleted++
				} else {
					LOG_DEBUG("SNAPSHOT_KEEP_PRUNE", "Keeping snapshot ID: %s, revision: %d, tag: %s, days: %d", snapshots[i].ID, snapshots[i].Revision, snapshots[i].Tag, days)
				}
			}
		} else if len(tags) > 0 {
			for _, snapshot := range snapshots {
