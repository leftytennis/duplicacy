...
func (manager *SnapshotManager) PruneSnapshots(selfID string, snapshotID string, revisionsToBeDeleted []int,
	...
	dryRun bool, deleteOnly bool, collectOnly bool, threads int, keepMax int) bool {

	LOG_DEBUG("DELETE_PARAMETERS",
		"id: %s, revisions: %v, tags: %v, keepMax: %d, retentions: %v, exhaustive: %t, exclusive: %t, "+
			"dryrun: %t, deleteOnly: %t, collectOnly: %t",
		snapshotID, revisionsToBeDeleted, tags, keepMax, retentions,
	...
	for id, snapshots := range allSnapshots {
		...
		if len(revisionsToBeDeleted) > 0 {
			...
			for i := 0; i < len(tags); i++ {
				...
				for k := 0; k < len(tag_snapshots)-keepMax; k++ {
			...
			for km, total := range keepMaxMap {
				LOG_INFO("SNAPSHOT_KEEP_MAX", "Keeping %d of %d snapshots for snapshot %s tag %s", MinInt(keepMax, total), total, km.ID, km.Tag)
			}
		} else if len(tags) > 0 {
			for _, snapshot := range snapshots {
				if _, found := tagMap[snapshot.Tag]; found {
